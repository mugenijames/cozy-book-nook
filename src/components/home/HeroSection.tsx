    ...
    <section className="relative overflow-hidden bg-gradient-to-br from-primary/5 via-background to-accent/10 py-16 md:py-24">
      ...
      <div className="container relative">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-6 animate-fade-up">
            ...
            <div className="flex flex-wrap gap-4 pt-2">
              ...
              <Button variant="outline" size="lg" asChild>
                <Link to="/books">View Bestsellers</Link>
              </Button>
            </div>
          ...
          {/* Right Book Stack */}
          <div className="relative hidden lg:flex justify-center items-end gap-4">
            {books.map((img, i) => (
              <div
                key={i}
                className={`relative group ${i !== 0 ? "-ml-8" : ""}`}
              >
                <div className="absolute inset-0 bg-primary/20 rounded-lg blur-2xl transform translate-y-8" />
                <img
                  src={img}
                  alt={`Book ${i + 1}`}
                  className="w-48 h-72 object-cover rounded-lg shadow-xl transition-all duration-300 group-hover:shadow-2xl group-hover:-translate-y-2"
                />
              </div>
            ))}
          </div>
